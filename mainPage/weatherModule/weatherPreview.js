...
const weatherPreview = async (city) => {
    ...
    weatherPreview.innerHTML = `
        ...
        <p class="block__description temperature">Температура воздуха: ${data.current.temp_c} °C.</p>
        <p class="block__description feelsLike">Ощущается: ${data.current.feelslike_c} °C.</p>
        <p class="block__description humidity">Влажность: ${data.current.humidity} %</p>
        <p class="block__description wind">Скорость ветра: ${data.current.wind_kph} км/ч.</p>
    `
